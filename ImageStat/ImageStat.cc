...
int main(const int argc, const char **argv)
{
	if (argc <=1 || ipExistsArgument(argv, "-usage") || ipExistsArgument(argv, "-help")) {
		...
		cout << "      -display   display the results on the standard output" << endl;
	...
	try
	{
		...
		if (labelOn){
			...
			efile<<"# LABEL \t VOLUME \t MEAN \t SD \t MIN \t MAX \t QUANTILES"<<endl;
			efile<<"# Fields :"<<endl;
			...
			efile<<"#\t SD            Standard deviation of those voxels"<<endl;
			...
			efile << "\t \t" << standardDeviation;
			efile << "\t \t" << Subsample->GetMeasurementVectorByIndex(0)[0];
			efile << "\t \t" << Subsample->GetMeasurementVectorByIndex(Subsample->Size()-1)[0];
			if(displayOn)
			{
				cout << l;
				cout << "\t \t" << nbPix*pixVol;
				...
				cout << "\t \t" << Subsample->GetMeasurementVectorByIndex(0)[0];
				cout << "\t \t" << Subsample->GetMeasurementVectorByIndex(Subsample->Size()-1)[0];
			}
			//quantiles
			...
			for(int i=0; i<tabSize; i++)
			{
				...
				{
					efile << "\t \t" << Subsample->GetMeasurementVectorByIndex((int)(floor(quantValue)))[0];
					if(displayOn) cout << "\t \t" << Subsample->GetMeasurementVectorByIndex((int)(floor(quantValue)))[0];
				}
				else
				{
					if (((quantValue - floor(quantValue)) == 0)||((quantValue - floor(quantValue)) < 0.5))
					{
						efile << "\t \t" << Subsample->GetMeasurementVectorByIndex((int)((floor(quantValue))-1))[0];
						if(displayOn) cout << "\t \t" << Subsample->GetMeasurementVectorByIndex((int)((floor(quantValue))-1))[0];
					}
					else
					{
						...
						{
							efile << "\t \t" << ((Subsample->GetMeasurementVectorByIndex((int)(floor(quantValue)) -1 )[0]) + (Subsample->GetMeasurementVectorByIndex((int)(floor(quantValue)))[0]))/2;
							if(displayOn) cout << "\t \t" << ((Subsample->GetMeasurementVectorByIndex((int)(floor(quantValue)) -1 )[0]) + (Subsample->GetMeasurementVectorByIndex((int)(floor(quantValue)))[0]))/2;
						}
					}
				}
			}
