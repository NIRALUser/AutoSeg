...
int main(int argc, const char* argv[])
{
  ...
  if (argc <= 1 || ipExistsArgument(argv,"-h ") || ipExistsArgument(argv,"-help") || 
	...
	<< "    -mask  <filename>     Mask to be saved"<<endl
      	<< " -WM <int>   White Matter Intensity Level (default: 1)"<<endl
      	<< " -GM <int>   Grey Matter Intensity Level (default: 2)"<<endl
      	<< " -CSF <int>  Cerebrospinal Fluid Intensity Level (default: 3)"<<endl
      	<< " -MWM <int>  Myelinated White Matter Intensity Level (default: -1, not taken into account)"<<endl
  ...
  if (skullstrippingOn && fileName.empty()) {
    ...
    if (skullstrippingOn && !fileName.empty()) {
	 ...
	 labelImage = labelReader->GetOutput() ;
       ...
       int mwm = (ipGetIntArgument(argv, "-MWM",-1));
	 ...
	 return EXIT_FAILURE;	
       ...
       if (mwm > 0)
	 {
	   //extract myelinated white matter
